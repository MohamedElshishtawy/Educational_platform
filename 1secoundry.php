...
if (isset($_SESSION['se']) && $_SESSION['se'] == '1se' && isset($_GET['id']) && $_GET['id'] == $_SESSION['id']) {
      ...
      if (!isset($_GET['to'])) { ?>
        ...
        <div class="group">
          ...
          <a class="to to-1" href="?id=<?php echo $_SESSION['id'] ?>&to=exams">
            <figure>
              <img src="<?php echo $imges . 'exam.svg' ?>" alt="you can't see this photo" class="se-img">
              <figcaption>
          ...
          <a class="to to-3" href="?id=<?php echo $_SESSION['id'] ?>&to=pdf">
            <figure>
          ...
          <a class="to to-4" href="?id=<?php echo $_SESSION['id'] ?>&to=images">
            <figure>
        ...
        if ( $to == 'video') {
          ...
          $videos_selected = select2('*', 'videos', 'se', '1se', ' && activity = 1', $no_vid_err);
          ...
          if ( $videos_selected != $no_vid_err) {
            foreach ($videos_selected as $video_selected) {
              ...
              // print_r( $fileTypePrepare_for_st);
          ...
          if(isset($_GET['img_dir'])){
            ...
            $all_images = select_info('*', 'images', 'se', '1se', ' && vialblity = 1 && name = "'.$images_name.'"');
            if($all_images != false ){
            ...
            $images_selected = select_info('name', 'images', 'se', '1se', ' && vialblity = 1');
          ...
          // get all exams in db
          $exams_db = select2('exam_name','exams','vilablility','1',' && se = "1se"');
          if ( $exams_db != false ) {
            ...
            $exams_file = scandir($exams_for_1);
              ...
              if ( count($prepared_exams) > 0 ) {
                ...
                foreach ( $prepared_exams as $prepare_exam ) {
                  ...
                  <a class="ex-btn col-lg-<?php echo $col ?>
                            ...
                            col-sm-<?php echo $col ?> 
                            col-sm-push-1 col-xs-10 col-xs-push-1" href="<?php echo $exams_for_1 . $prepare_exam; ?>">
                    <span>
                      <?php
          ...
          if ( isset($_GET['pdf']) ) {
            ...
            $get_pdf = select2('*','pdf','pdf_id',$pdf_id,' && se = "1se"');
            if ( $get_pdf != false ) {
            ...
            $select_pdf = select2('pdf_id,pdf,name','pdf','se','1se',' && vilablity = 1');
            if ( $select_pdf != false ) {
                // if condition for style only
                $push_pdf = 1;
                $col_pdf = 5;
