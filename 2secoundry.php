...
if (isset($_SESSION['se']) && $_SESSION['se'] == '2se' && isset($_GET['id']) && $_GET['id'] == $_SESSION['id']) {
    ...
    if ($monies['0']['money'] == 0) {
      ...
      if (!isset($_GET['to'])) { ?>
        ...
        if ($_GET['to'] == 'video') {
          ...
          $videos_selected = select2('*', 'videos', 'se', '2se', ' && activity = 1', $no_vid_err);
        ...
        ?>
          ...
          if(isset($_GET['img_dir'])){
            ...
            $all_images = select_info('*', 'images', 'se', '2se', ' && vialblity = 1 && name = "'.$images_name.'"');
            if($all_images != false ){
            ...
            $images_selected = select_info('name', 'images', 'se', '2se', ' && vialblity = 1');
          ...
          if ( $exams_db != false ) {
            ...
            $exams_file = scandir($exams_for_2);
              ...
              if ( count($prepared_exams) > 0 ) {
                ...
                foreach ( $prepared_exams as $prepare_exam ) {
                  ...
                  <a class="ex-btn col-lg-<?php echo $col ?>
                            ...
                            col-sm-<?php echo $col ?> 
                            col-sm-push-1 col-xs-10 col-xs-push-1" href="<?php echo $exams_for_2 . $prepare_exam; ?>">
                    <span>
                      <?php
          ...
          if ( isset($_GET['pdf']) ) {
            ...
            $get_pdf = select2('*','pdf','pdf_id',$pdf_id,' && se = "2se"');
            if ( $get_pdf != false ) {
            ...
            $select_pdf = select2('pdf_id,pdf,name','pdf','se','2se',' && vilablity = 1');
            if ( $select_pdf != false ) {
                // if condition for style only
                $push_pdf = 1;
                $col_pdf = 5;
