...
if (!isset($_SESSION['phone']) || !$_SESSION['se'] == '1se' || !$_SESSION['se'] == '2se' || !$_SESSION['se'] == '3se') {
    ...
    if ($last_value_http != '&to=exams' || $last_value_my_link[0] != $exam_id . '.php') {
    ...
    if ( $time_history ) { // There is Time in history
        ...
        elseif( ($duration -= $time_now - $started_time) <= 0){ // time run out
            $submit = true;
            $delet_time_history = $db->prepare("DELETE FROM timer WHERE student_id = $student_id");
            $delet_time_history->execute();
        }
    }else{
        $set_timer = $db->prepare("INSERT INTO timer (student_id,start_time,exam_id) VALUES (?,?,?) ");
        $put_time_hist->execute(array($student_id, $time_now_new, $exam['id']));
    }
    ...
    //     // Loop for every qestion to collect the answer only
    ...
    //     // INSERT THE DEGREE
    //     $deg = $score . ' / ' . $ans_count;
    //     $save_deg = $db->prepare("INSERT INTO degrees (student_id, exam_id, degree) VALUES (? , ? , ?)");
    ...
    //     } catch (Exception $e) {
    //         message("There is an error<br>Please take a screen for this message and send it for us <br>" . $e, "danger", "../");
    //     }
    ...
    //       }
    //     }
    //     message("تم حل الإمتحان الحمد لله", "success", "../");
    // } else {
    //     // Open the exam
    //     $open_exam = true;
    ...
    //                 if ($time_history[0]['for_exam'] == $exam['id']) {
    ...
    //                 $time_you_have = (int)$exam['duration'] * 60;
    ...
    //                 <div class="exam-nav" id="timer">
    //                     <span class="min" id="min"></span>
    //                     <progress class="sec" id="sec" max="<?= $time_you_have /*sec*/ ?>" value=""></progress>
    //                 </div>
    //             <?php
    //                 $z = 0;
    //                 echo '<ol>';
    ...
    //                     <div>الإمتحان: <b><?php echo str_replace('_', ' ', $time_history['for_exam']); ?></b></div>
    //                     <div>المعلومات</div>
    //                     <ol>
    //                         <li>وقتك المتبقي لك</li>
    //                         <li>الأجابات التي اخترتها</li>
